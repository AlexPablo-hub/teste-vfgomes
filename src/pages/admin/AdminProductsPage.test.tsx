...
// Fixtures locais — substituem os antigos `mockProducts` agora que a fonte
// real é a Fakestore API. Usadas pra simular estado pré-hidratação ou modo
// offline forçado nos testes.
const testProducts: Product[] = [
  {
    id: 1,
    title: "Raven Silk Pumps",
    price: 1290,
    description: "Scarpins em seda natural com bico fino.",
  ...
  },
  {
    ...
    price: 8900,
    description: "Cronógrafo automático em aço com calibre suíço.",
...
describe("AdminProductsPage", () => {
  beforeEach(() => {
    // Cada teste começa com store seedado com fixtures locais e localStorage limpo.
    useProductsStore.setState({ products: testProducts, hydratedAt: null });
  });

  it("renderiza heading e botão Adicionar Produto", async () => {
    renderWithProviders(<AdminProductsPage />, {
      initialRoute: "/admin/estoque"
    });
    expect(
  ...
  it("lista produtos do store inicialmente (mocks NOIR LUXE)", async () => {
    renderWithProviders(<AdminProductsPage />, {
      initialRoute: "/admin/estoque"
    });
  ...
  it("mantém produtos no store quando hidratação falha (modo offline)", async () => {
    ...
    // Mocks NOIR LUXE continuam visíveis (fallback offline)
    expect(await screen.findByText("Midnight Chronograph")).toBeInTheDocument();
  });

  it("filtra produtos por busca", async () => {
    // Força modo offline pra preservar mocks NOIR LUXE no store durante o teste
    server.use(http.get(`${BASE}/products`, () => HttpResponse.error()));
  ...
  it("abre modal de novo produto ao clicar Adicionar Produto", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: /novo produto/i })
    ).toBeInTheDocument();
    expect(
  ...
  it("cria produto: chama POST /products + adiciona ao store + toast success", async () => {
    ...
    await user.type(
      screen.getByLabelText(/url da imagem/i),
      "/images/test.jpg"
    );
    await user.type(
  ...
  it("exclui produto: chama DELETE /products/:id + remove do store + toast", async () => {
    // Força offline pra manter os mocks NOIR LUXE (id=2 = Midnight Chronograph)
    server.use(http.get(`${BASE}/products`, () => HttpResponse.error()));
    ...
    const deleteButton = screen.getByRole("button", {
      name: /excluir midnight chronograph/i
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Midnight Chronograph")
